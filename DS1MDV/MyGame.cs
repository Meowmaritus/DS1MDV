...
namespace XnaTestThing
{
    public class MyGame : Game
    {
        ...
        public MyGame()
        {
            ...
            IsMouseVisible = true;
            base.Window.Title = "Meowmaritus's Dark Souls 1 Mesh Data Viewer";
        }
        ...
        private void LoadModel()
        {
            ...
            for (int i = 0; i < inputFiles.Length; i++)
            {
                ...
                if (fileType.Contains("DCX"))
                {
                    ...
                    {
                        var bnd = DataFile.LoadFromDcxFile<BND>(file);
                        //bnd.CreateBackup();
                        flvers[i] = bnd.Entries.Where(x => x.Name.ToUpper().EndsWith(".FLVER")).First().ReadDataAs<FLVER>();
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        var bnd = DataFile.LoadFromFile<BND>(file);
                        //bnd.CreateBackup();
                        flvers[i] = bnd.Entries.Where(x => x.Name.ToUpper().EndsWith(".FLVER")).First().ReadDataAs<FLVER>();
                    }
                    else
        ...
        private void BuildModelPrimitives(int f)
        {
            ...
            for (int i = 0; i < flvers[f].Submeshes.Count; i++)
            {
                ...
                for (int j = 0; j < flvers[f].Submeshes[i].Vertices.Count; j++)
                {
                    ...
                    //    //primitiveDatas[f][i][j].Normal =
                    //    //    Vector3.Cross((Vector3)vert.Normal, new Vector3(vert.BiTangent.X, vert.BiTangent.Y, vert.BiTangent.Z))
                    //    //    * vert.BiTangent.W;

                    //    //primitiveDatas[f][i][j].Normal = Vector3.Cross(new Vector3(vert.BiTangent.X, vert.BiTangent.Y, vert.BiTangent.Z), primitiveDatas[f][i][j].Normal);

                    //    if (vert.BiTangent.W == -1)
                    //    {
                    //        primitiveDatas[f][i][j].Normal =
                    //            Vector3.Cross((Vector3)vert.Normal, new Vector3(vert.BiTangent.X, vert.BiTangent.Y, vert.BiTangent.Z))
                    //            * vert.BiTangent.W;
                    ...
                    //    primitiveDatas[f][i][j].Tangent = new Vector3(vert.BiTangent.X, vert.BiTangent.Y, vert.BiTangent.Z);
                    //}
            ...
            {
                cameraPosition = cameraPositionDefault = new Vector3(0, (MAX_VERTEX_Y / -2), -(MAX_VERTEX_Y / 2));
            }
        }
        ...
        private void CreateModelListWindow()
        {
            ...
            ModelListWindow.BoneNames = flvers[0].Bones.Select(x => x.Name).ToList();
            ModelListWindow.BoneScales = flvers[0].Bones.Select(x => x.Scale).ToList();
        ...
        {
            if (ModelListWindow != null && ModelListWindow.IsVisible)
            {
